"""
MetaTrader5 for macOS - A reverse-engineered macOS-compatible implementation

This package provides macOS compatibility for the MetaTrader5 Python API by
implementing a socket-based IPC bridge to communicate with MT5 running on
Windows (via Wine/CrossOver) or a Windows VM.
"""

__version__ = '5.0.5735'
__author__  = 'MetaQuotes Ltd. / macOS Port'

import os
import sys

# Platform check - this version is for macOS
if sys.platform == 'win32':
    raise ImportError("This is the macOS version of MetaTrader5. Please use 'pip install metatrader5' for Windows.")
...
# Import the core module which handles IPC communication
from ._core import (
    account_info,
    initialize,
    shutdown,
...
# High-level helper functions (same as Windows version)

def _RawOrder(order_type, symbol, volume, price, comment=None, ticket=None):
    """Internal helper to send a trade order."""
    order = {
...
def Close(symbol, *, comment=None, ticket=None):
    """
    Close a position by symbol or ticket.

    Args:
        symbol: Symbol name (e.g., "EURUSD")
        comment: Optional comment for the close order
        ticket: Optional position ticket to close specific position

    Returns:
        True if all positions closed successfully
        "Partially" if some positions closed
        False if no positions closed
        None on error
    """
    if ticket is not None:
...
def Buy(symbol, volume, price=None, *, comment=None, ticket=None):
    ...
    Args:
        symbol: Symbol name (e.g., "EURUSD")
        volume: Order volume
        price: Optional specific price (uses current ask if None)
        comment: Optional order comment
        ticket: Optional position ticket for modify operations

    Returns:
        OrderSendResult object or None on error
    """
    # with direct call
...
def Sell(symbol, volume, price=None, *, comment=None, ticket=None):
    ...
    Args:
        symbol: Symbol name (e.g., "EURUSD")
        volume: Order volume
        price: Optional specific price (uses current bid if None)
        comment: Optional order comment
        ticket: Optional position ticket for modify operations
